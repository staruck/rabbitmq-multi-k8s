...
if [ -z "$ORG_NAME" ] || [ -z "$NAMESPACE_RABBIT" ] || [ -z "$REGION02" ] || [ -z "$DOMAIN" ]; then
...
COMMON_NAME="${NAMESPACE_RABBIT}-${REGION02}.${DOMAIN}"

# Define o arquivo de configuração original e o novo local
CONFIG_FILE="./models/openssl.cnf"
NEW_CONFIG_FILE="./certificates/${NAMESPACE_RABBIT}-${REGION02}/openssl.cnf"
